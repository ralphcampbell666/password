 ...
 * @author ralph
 */
public class safe_password_undelete extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        else 
        {
            ...
            try 
            {
                ...
                String password_id = history_password[4];//password_id,  
                
                safe_id = history_password[5];
                ...
                stmt.setString(2, password_history_safe_id);
                stmt.setString(3, password_history_name);
                stmt.setString(4, password_history_description);
                stmt.setString(5, password_history_password);
                stmt.setString(6, password_history_expires_on);
                stmt.setString(7, password_history_date_created);
                stmt.setString(8, password_history_creator_user_id);
                stmt.execute();
                //add a history record
                boolean results = db.password.add_password_history_record(con, session, "Undelete", String.valueOf(password_id), password_history_safe_id, password_history_name, password_history_description, password_history_password, password_history_expires_on, password_history_date_created, password_history_creator_user_id);
                con.close();
                
            }
            catch(SQLException e)
            {
                System.out.println("Exception in admin_password_undelete servlet=" + e);
            } 
        }
